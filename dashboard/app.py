...
st.markdown("""
<style>
/* ── Dana Point PULSE loading splash ─────────────────────────────────────── */
@keyframes spl-rise  { from{opacity:0;transform:translateY(22px)} to{opacity:1;transform:translateY(0)} }
@keyframes spl-load  { from{width:0%} to{width:100%} }
@keyframes spl-pulse { 0%,100%{opacity:1} 50%{opacity:0.55} }
@keyframes spl-glow  { 0%,100%{text-shadow:0 0 24px rgba(56,189,248,0.0)} 50%{text-shadow:0 0 48px rgba(56,189,248,0.65),0 0 80px rgba(56,189,248,0.3)} }
#pulse-splash {
...
#pulse-splash.spl-out { opacity:0 !important; visibility:hidden !important; pointer-events:none !important; }
</style>
<div id="pulse-splash">
  <div style="
    font-family:'Syne','Outfit',Arial Black,sans-serif !important;
    font-size:clamp(2rem,5vw,3.5rem) !important;
    font-weight:900 !important;
    letter-spacing:-0.04em !important;
    color:#FFFFFF !important;
    opacity:0;
    animation:spl-rise 0.6s 0.1s cubic-bezier(0.22,1,0.36,1) forwards;
    white-space:nowrap;
  ">Dana Point&nbsp;<span style="color:#38BDF8 !important;animation:spl-glow 2s 0.7s ease-in-out infinite;">PULSE</span></div>
  ...
  <div style="
    ...
    opacity:0;
    animation:spl-rise 0.6s 0.28s cubic-bezier(0.22,1,0.36,1) forwards;
  ">Destination Intelligence Platform</div>
  ...
  <div style="
    ...
    opacity:0;
    animation:spl-rise 0.4s 0.42s ease forwards;
  ">
    <div style="
      height:100%; width:0%;
      background:linear-gradient(90deg,#0567C8,#38BDF8,#7DD3FC);
      border-radius:2px;
      animation:spl-load 1.4s 0.55s cubic-bezier(0.4,0,0.2,1) forwards;
    "></div>
  </div>
  ...
  <div style="
    ...
    opacity:0;
    animation:spl-rise 0.4s 0.7s ease forwards, spl-pulse 1.8s 1.2s ease-in-out infinite;
  ">LOADING ANALYTICS ENGINE…</div>
</div>
<script>
(function(){
  var _sp = null;
  function getSP(){ return _sp || (_sp = document.getElementById('pulse-splash')); }
  function hideSplash(){
    var sp = getSP(); if(!sp) return;
    sp.classList.add('spl-out');
    setTimeout(function(){ if(sp && sp.parentNode) sp.style.display='none'; }, 800);
  }
  // Hard cutoff — always dismiss after 2.2s regardless
  setTimeout(hideSplash, 2200);
  // Also dismiss as soon as Streamlit's main content appears
  var _obs = new MutationObserver(function(){
    if(document.querySelector('[data-testid="stMainBlockContainer"], .main .block-container')){
