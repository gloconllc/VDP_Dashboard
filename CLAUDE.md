...
- **A "commit to git" step is not the same as "map into the schema."** The "Upload Your Data" panel (added 2026-08-17) dispatched `str_weekly_sync.yml` after every upload regardless of source type, but that workflow only calls the STR loaders plus `compute_kpis.py`/`compute_insights.py`. An uploaded Datafy, CoStar, or Later.com file would commit cleanly and land in the right `data/<source>/` folder, then sit there forever unparsed, with no error surfaced to the person who uploaded it. Before shipping any "upload updates the dashboard" feature, trace the exact dispatched job to confirm it actually calls the loader for every source type the UI offers, not just the one the workflow happened to be built for. Removed 2026-08-18; see "Client Self-Serve Upload — Removed 2026-08-18" above.
- **CoStar's raw Excel export naming drifted again** (fixed `daily_costar.xlsx`/`monhtly_costar.xlsx` → dated drops like `Daily_08_26_26.xlsx`/`Monthly_08_26_26.xlsx`/`Particpation_08_26_26.xlsx`), the same pattern already documented for Datafy above. `load_costar_market_daily.py` was hardcoded to the two fixed filenames and silently never saw the dated ones. Fixed 2026-08-31 with `_latest_costar_export()`, which globs `Daily_*.xlsx`/`Monthly_*.xlsx`, picks the newest by the `MM_DD_YY` embedded in the filename (falling back to mtime, then to the legacy fixed name), so a fresh dated drop loads automatically with no rename step. `costar_market_daily`/`costar_market_monthly` both already have `UNIQUE(...) ON CONFLICT REPLACE`, so re-running this loader is always safe.
- **The new `Particpation_08_26_26.xlsx` CoStar export (note the source's own typo, "Particpation") is a real per-hotel monthly participation roster** (13 Dana Point-area properties × Jul 2024–Jul 2026, room counts + a participating/non-participating marker per month) with no loader yet. This looks like the legitimate real-data replacement for the fabricated `costar_competitive_set.csv` baseline flagged elsewhere in these lessons, worth a proper loader in a future pass rather than building blind under a "just refresh the data" task.
- **A stray 24-byte `.xls` file is a corrupted/placeholder download, not real data, and this keeps recurring across both `data/str/` and `data/costar/`.** Confirmed again 2026-08-31: `VisitDanaPoint_202605.xls`, `_20260531.xls`, `_202606.xls` (×2), `_20260719.xls` (×4 variants), `_20260726.xls` (×3 variants) in `data/str/`, plus `VisitDanaPoint_202607.xls` and `_20260802.xls` in `data/costar/`, all exactly 24 bytes. Never load or count these; the underlying cause (likely the Dropbox sync grabbing an error page instead of the real file, see the `fetch_str_dropbox.py` lesson above) is still unresolved and worth a dedicated look since it is not a one-off.

- **A PDF-page thumbnail is a picture of a chart, not a chart.** Every "View a Section" card showed only a rasterized preview of its PDF page, which at card width is unreadable, carries no hoverable value, and still reads to a viewer as "this dashboard has no visualizations." `dashboard/section_visuals.py` now supplies a live, card-sized Plotly figure plus a sourced one-line caption for each of the seven non-summary sections, read straight from `analytics.sqlite`, with the thumbnail kept only as the fallback when a section has no chart of its own (Executive Summary, Notes) or its source table is empty. Card-sized figures need their own layout rules, not a shrunken full-page chart: one series, value labels drawn on the marks instead of on an axis, no gridlines, no legend.
- **Long category/DMA names on a horizontal bar chart need a transparent tick, not just `automargin`.** plotly 5 has no `ticklabelstandoff`, so `automargin=True` alone leaves the label right-edge touching the bar. `ticks="outside", ticklen=5, tickcolor="rgba(0,0,0,0)"` buys real separation without drawing a tick mark.
- **A subset series must not look stacked.** Compression days above 90% are a subset of days above 80%, but two equal-width `barmode="overlay"` bars read as a stack and imply a bogus total. Give the subset series an explicitly narrower `width` (0.30 against 0.68) so it reads as an inset marker inside its parent bar.
- **The feeder-market map is now a tiled-basemap flow map, and that adds a network dependency the old one did not have.** `go.Scattermap` (plotly 5.24+, MapLibre) on `carto-positron` replaces the flat `Scattergeo` USA outline, with bubbles graduated by spend share and connector lines to Dana Point weighted by the same value. Two things to know: MapLibre fetches its style JSON and glyphs from a CDN, so if that fetch fails the map renders blank, markers included, not just missing tiles; and text labels on a `scattermap` trace need the style's glyph source, so the "Dana Point" label will not draw on `white-bg`. `build_markets_map_figure` in `app.py` is kept as the no-network fallback and `build_feeder_market_map` returning None falls back to it. A blank map in a sandbox screenshot means blocked tiles, not broken trace geometry: re-test with `style="white-bg"` to confirm the markers themselves are correct.
- **`streamlit.testing.v1.AppTest` can be run against a purpose-built harness instead of the whole app.** `dashboard/app.py` pulls in `generate_weekly_report` and therefore WeasyPrint's system libraries, which will not install in every environment. A 40-line harness app that imports only the module under test, calls the same `st.plotly_chart(..., key=...)` signature, and prints a render count gives a real zero-exception verification of the new code path under real Streamlit caching without needing the PDF toolchain. Confirmed 7 of 7 cards plus the map at zero exceptions.
- **The whole app still calls `use_container_width`, which Streamlit has scheduled for removal after 2025-12-31** in favor of `width="stretch"` / `width="content"`. Every `st.plotly_chart` / `st.image` / `st.button` / `st.download_button` call site logs a deprecation warning on a current Streamlit. Not urgent while `requirements.txt` pins `<2.0.0`, but it is a real, dated removal and deserves its own pass before the pin is raised.

- **`group_intelligence`'s dollar estimates are downstream of a table CLAUDE.md already flags as fabricated.** Its `data_sources` column literally names `costar_chain_scale_breakdown`, and every row's own `group_displacement_note`/`notes` says "Benchmark data only, STR group-segment data n[ot available]." When building the new Forward Outlook & Group Business main-page section, this ruled out surfacing `group_intelligence`'s estimated group room revenue / TBID / TOT figures as if they were vetted Layer 1 data. Built the section from real sources only instead: `fact_str_group_metrics` (live weekly STR segment mix), `kpi_compression_quarterly` (real STR compression), the seeded `vdp_events` calendar, and `insights_daily` (pipeline-generated from real STR+Datafy). Before adding a new client-facing metric, check what a table's own source/notes columns say about itself, not just its schema.
- **CoStar's multi-year annual-performance table mixes two different PDFs' report_date under one query unless you filter for it.** `costar_annual_performance` holds both the submarket historical PDF (years 2016-2025 + YTD, `report_date=2026-08-26`) and an older separate forecast PDF (years 2026-2030, `report_date=2026-08-11`, all pace/forecast rows). A trend query across "all years" without pinning `report_date` to the row that actually has `year_label='YTD'` silently blends a real historical series with unrelated forecast rows. `costar_overall_trend_df()` in `section_visuals.py` pins to that report_date and also drops the current, in-progress year's own numeric row (see the existing "CoStar's own multi-year forecast tables" lesson above) in favor of the real YTD actual.
- **A card-sized figure and a main-page figure can share one query and one builder function if height becomes a parameter.** Added `height:` kwargs to `_fig_costar_tiers`, `_fig_room_split`, `_fig_compression`, and `_fig_group_mix` in `section_visuals.py` rather than writing bigger duplicate versions for the new Market Performance / Forward Outlook main-page sections; font size and legend visibility branch on `height <= 220` vs. bigger. One thing this did NOT anticipate: a horizontal legend needs real top margin, not just `showlegend=True` -- at the card's `t=6` margin the legend clips invisibly against the container edge. Fixed by giving `_fig_compression` an explicit `t=34` margin override whenever `height > 220`.
- **A ranked horizontal bar chart is not the same thing as a categorical pie's legend-linked colors, so cycling a palette by rank position is safe and reads better than a single hue.** The feeder-market bars (both the card and the main-page "Top Visitor Origin Markets" chart) went from solid teal to one palette color per bar. Getting the color-to-bar mapping right required tracking each chart's own sort direction and y-axis orientation separately: the card sorts ascending with no axis reversal (so the palette list needs `[::-1]` to keep rank 1 mapped to the first palette color at the visual top), while the main-page chart is already sorted rank-descending with `yaxis=dict(autorange="reversed")` (so no reversal is needed, and adding one gets the mapping backwards). Verified by printing the literal `(market_name, color)` pairs before trusting a screenshot.
- **`fitz`/PyMuPDF's `doc.select(indices)` is the simplest way to build a filtered, reordered PDF from an existing one**, and it is already a project dependency for the flipbook rasterizer, so no new package was needed for the "export selected sections" feature. `doc.select()` mutates in place and keeps the *given* order, not the original page order, so the caller must pass already-sorted indices. Verified against a synthetic multi-page PDF (built with `fitz.open()` + `new_page()`) rather than the real WeasyPrint report, since generating a real report inside a bare test harness pulls in the full report pipeline; the synthetic-PDF approach is fast and exercises the real `select()`/`tobytes()` code path faithfully.
- **`streamlit.testing.v1.AppTest` catches widget-key collisions and stale-selection bugs that a plain function-level test cannot.** The harness added for the two new main-page sections plus the flipbook section-picker exercised the full default-selection run (0 exceptions, 10/10 pages, all metrics/charts correct against real data) and then called `.set_value([...]).run()` on the multiselect to simulate a user narrowing their choice, confirming the filtered page count, the subset PDF's non-zero byte length, and zero exceptions on the second run too. A single default-state test would have missed a bug in the "user changed the selection" branch entirely.
...
## Client Self-Serve Upload — Removed 2026-08-18

An "Upload Your Data" panel briefly existed in `app.py` (added 2026-08-17, removed the next day). It saved a client-uploaded STR/Datafy/CoStar/Later file into the correct canonical `data/<source>/` folder, then, only if `GITHUB_TOKEN` was set, committed it to `VDP_Dashboard` main via GitHub's Contents API and dispatched `str_weekly_sync.yml` via the Actions API.

**Why it was removed:** on inspection, `str_weekly_sync.yml` (the only workflow the panel dispatched) runs `load_str_daily_sqlite.py`, `load_str_monthly_sqlite.py`, `load_str_multiseg.py`, `compute_kpis.py`, `compute_insights.py`, `optimize_db.py`, and `build_table_relationships.py`. It does not call `load_datafy_reports.py`, any CoStar loader, or `load_later_reports.py`. So an uploaded Datafy, CoStar, or Later.com file would land correctly in its `data/<source>/` folder and commit cleanly to git, but nothing would ever parse it into `analytics.sqlite`. It would sit as an unprocessed raw file, silently, with no error shown to the uploader, until someone noticed and ran the full pipeline by hand. Only an uploaded STR file was ever actually mapped end to end. That is not a clean, source-agnostic mapping into the schema and `table_relationships`, it is a raw file drop that happens to work for one of five source types.

**Current approach instead:** data intake stays controlled through the agreed shared folders, loaded via the Standard Process above (raw file to `data/<source_name>/`, then the matching `load_<source>.py`, then `run_pipeline.py`). If a true self-serve upload is wanted again later, it needs a per-source-type dispatch (or a workflow that always runs the full monthly pipeline, not just the STR-only weekly sync) so every file type is actually loaded, not just committed.
...
| 2026-08-18 | Added notes and archive documentation to the in-app "How to Use This Report" help panel. Then, on review, removed the "Upload Your Data" panel entirely after tracing its dispatched workflow (`str_weekly_sync.yml`) and confirming it only runs the STR loaders, not `load_datafy_reports.py`, any CoStar loader, or `load_later_reports.py`. An uploaded Datafy, CoStar, or Later.com file would have committed to the repo but never been parsed into `analytics.sqlite`, no error shown. Removed the panel, its two now-unused imports (`re`, `requests`), and its jump-nav link; documented the reasoning under "Client Self-Serve Upload — Removed 2026-08-18" above and in Lessons Learned. Data intake goes back to the agreed shared folders and the Standard Process pipeline until a source-aware upload path is built. Verified with AppTest, zero exceptions. | Claude |
| 2026-08-31 | Refreshed CoStar data: fixed `load_costar_market_daily.py`, which was hardcoded to `daily_costar.xlsx`/`monhtly_costar.xlsx` and never saw the newer dated exports (`Daily_08_26_26.xlsx`, `Monthly_08_26_26.xlsx`), to auto-pick the newest dated file by filename (falls back to the legacy fixed name). Reran the CoStar market-daily loader (costar_market_daily now through 2026-08-22, costar_market_monthly through Jul 2026) and load_costar_reports.py (parsed the new Aug 26 Newport Beach/Dana Point submarket and US national PDFs), then compute_kpis, compute_insights (48 fresh insights), build_table_relationships, and optimize_db, all clean. Flagged two items for a future pass rather than building blind: the new `Particpation_08_26_26.xlsx` CoStar file is a real per-hotel participation roster with no loader yet (likely the real replacement for the fabricated `costar_competitive_set.csv` baseline); and a batch of 24-byte corrupted placeholder `.xls` downloads keeps recurring in both `data/str/` and `data/costar/`, still unresolved at the sync-source level. Left the new Datafy_08_28_26 batch (37 CSVs, including 6 new advertising-performance file types) unloaded pending John's answers on its real date range and on what distinguishes several duplicate-numbered files, since the period-stamping fallback would otherwise mislabel it over real 2025 annual data. | Claude |
| 2026-08-31 | Data refresh plus a real visualization pass on the report page. Refreshed CoStar (market daily through 2026-08-22, monthly through Jul 2026, the 08/26 submarket and national PDFs), recomputed KPIs, generated 48 insights, rebuilt 366 table relationships, and re-optimized the DB. Patched `load_costar_market_daily.py` with `_latest_costar_export()` so a dated CoStar drop (`Daily_08_26_26.xlsx`) is picked up automatically instead of only the legacy fixed filenames. New `dashboard/section_visuals.py` gives each of the seven non-summary "View a Section" cards a live, card-sized Plotly figure and a sourced caption in place of the PDF-page thumbnail: occupancy by day of week, compression days by quarter with the 90%-plus subset inset, top feeder markets, CoStar chain-scale RevPAR by tier, submarket room inventory by tier, visitor spend by category, and the Dana Point transient/group occupancy mix. Upgraded the feeder-market map from a flat `Scattergeo` state outline to a MapLibre `Scattermap` flow map on `carto-positron`, bubbles graduated by spend share with connector lines weighted to match, `build_markets_map_figure` retained as the no-network fallback. Verified with a purpose-built `AppTest` harness against the real database: 7 of 7 cards and the map, zero exceptions. Datafy_08_28_26 (37 CSVs, 6 new advertising-performance report types) is still held out of the pipeline pending its real date range and the duplicate-numbered-file question, since loading it blind would overwrite the 2025 annual period key. | Claude |
| 2026-08-31 | Second visualization pass, same day: added two new full main-page sections, 🏢 Market Performance (CoStar submarket KPIs, RevPAR-by-tier bar, room-inventory donut, and a new multi-year RevPAR trend line, all real parsed-PDF CoStar data) and 📈 Forward Outlook & Group Business (next major event, this quarter's compression, STR group occupancy share, the group occupancy-mix chart, the compression-by-quarter chart, and a callout quoting today's top-priority insights_daily row), both reusing the section-card query/figure functions in `section_visuals.py` via new `height` parameters rather than duplicating logic. Deliberately excluded `group_intelligence`'s dollar projections, which trace back to the already-flagged fabricated `costar_chain_scale_breakdown` table. Colorized the top-origin-markets bar chart (one palette color per market, both the card and the main-page version) in place of solid teal. Added a section picker above "Flip Through the Full Report": a multiselect defaulting to every section (matching prior behavior exactly), which filters the flipbook and unlocks a new "Download Selected Sections as PDF" export built with `fitz`'s `doc.select()`. Verified with an extended AppTest harness against the real database, including a simulated selection change: zero exceptions in both the default (10/10 pages) and narrowed (3/10 pages, real non-empty subset PDF) states. | Claude |
